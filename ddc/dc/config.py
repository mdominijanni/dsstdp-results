...
_config_template = """
[meta]
name = "dc"                            # name of the configuration, directory name, and used by WandB
...
inplace = false                        # use in-place operations for storing synapse values
...
def _ingest_current_version(confd: dict[str, Any]) -> SimpleNamespace:
    ...
    config.model.trainerargs = {
        "lr_post": confd["training"]["lr_post"],
        "lr_pre": confd["training"]["lr_pre"],
        "tc_post": confd["training"]["tc_post"],
        "tc_pre": confd["training"]["tc_pre"],
        "batch_reduction": confd["training"]["batch_reduction"],
    }
