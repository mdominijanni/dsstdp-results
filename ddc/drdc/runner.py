...
from .modelling import build_weight_trainer, build_delay_trainer
import numpy as np
import torch
...
def build_trainers(network: dcshared.DCSNN, cfg: SimpleNamespace) -> dcshared.TrainerGroup:
    with torch.no_grad():
        return dcshared.TrainerGroup(
            weight_trainer=build_weight_trainer(network, **cfg.model.weighttrainerargs),
            delay_trainer=build_delay_trainer(network, **cfg.model.delaytrainerargs),
        )
...
def build_environ(cfg: SimpleNamespace) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        cfg=cfg,
        m=m,
        trainers=build_trainers(m.network, cfg).to(
...
def clear(env: SimpleNamespace) -> None:
    ...
    env.trainers.clear(keepshape=False)
        ...
        for epoch in tqdm(
            ...
            if env.prog.train < env.cfg.data.ntrain:
                ...
                for X, y in tqdm(
                    ...
                    exc, inh = env.m.network(
                        encoded, [env.trainers.weight, env.trainers.delay], env.batchlog
                    )
                    clear(env)
