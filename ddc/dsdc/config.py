...
_config_template = """
[meta]
name = "dsdc"                          # name of the configuration, directory name, and used by WandB
...
[training.common]                      # training options which apply to delays and weights
tc_post = 30.0                         # time constant of exponential decay for postsynaptic traces (in ms), triggered on pre
tc_pre = 30.0                          # time constant of exponential decay for presynaptic traces (in ms), triggered on post
batch_reduction = "mean"               # batch axis reduction, can be one of: "mean", "sum", "max", "median", ["quantile", q], ["quantile", q, interp], "geomean"

[training.weight]
lr_post = 5e-4                         # postsynaptic LR for weights with STDP
lr_pre = -5e-6                         # presynaptic LR for weights with STDP

[training.delay]
lr_post = -5e-4                        # postsynaptic LR for delays with STDP
lr_pre = 5e-6                          # presynaptic LR for delays with STDP
...
def _ingest_current_version(confd: dict[str, Any]) -> SimpleNamespace:
    ...
    config.model.trainerargs = {
        "weight_lr_post": confd["training"]["weight"]["lr_post"],
        "weight_lr_pre": confd["training"]["weight"]["lr_pre"],
        "delay_lr_post": confd["training"]["delay"]["lr_post"],
        "delay_lr_pre": confd["training"]["delay"]["lr_pre"],
        "tc_post": confd["training"]["common"]["tc_post"],
        "tc_pre": confd["training"]["common"]["tc_pre"],
        "batch_reduction": confd["training"]["common"]["batch_reduction"],
        "inplace": confd["runtime"]["inplace"],
    }
